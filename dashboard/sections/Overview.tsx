...
const TrafficLineChart = ({ data }: { data: ChartDataPoint[] }) => {
  ...
  const points = data.map((d, i) => ({
    x: (i / (data.length - 1)) * (viewBoxWidth - padding * 2) + padding,
    y: viewBoxHeight - ((d.value / max) * (viewBoxHeight - padding * 2)) - padding
  }));

  const pathData = `M ${points.map(p => `${p.x},${p.y}`).join(' L ')}`;
  const areaData = `${pathData} L ${points[points.length - 1].x},${viewBoxHeight} L ${points[0].x},${viewBoxHeight} Z`;
    ...
    <div className="relative w-full h-[200px] mt-4">
      ...
      >
        ...
        {[0, 0.5, 1].map((p, i) => (
          <line
            key={i}
            x1={padding} y1={viewBoxHeight * p || padding}
            x2={viewBoxWidth - padding} y2={viewBoxHeight * p || padding}
            stroke="currentColor"
      ...
      <div className="flex justify-between mt-4 px-4 overflow-x-auto scrollbar-hide">
        {data.map((d, i) => (
          <span key={i} className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase whitespace-nowrap">{d.label}</span>
        ))}
      </div>
    </div>
  );
      ...
      <div className="bg-white dark:bg-[#0f172a] rounded-3xl border border-slate-200 dark:border-white/5 p-6 shadow-xl relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {[
              { label: 'Uptime', value: stats?.uptime, icon: ShieldCheck, color: 'emerald' },
              { label: 'Active Leads', value: '14', icon: TrendingUp, color: 'primary' },
              { label: 'Pending Deploy', value: '03', icon: Zap, color: 'amber' }
            ].map((m, i) => (
              <div key={i} className="bg-white dark:bg-[#0f172a] p-6 rounded-3xl border border-slate-200 dark:border-white/5 shadow-sm">
                <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-3">{m.label}</div>
                <div className="flex justify-between items-end">
                  <div className="text-3xl font-bold dark:text-white">{m.value}</div>
            ...
            {logs.map((log) => (
              <div key={log.id} className="p-5 flex items-center justify-between group hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">
                <div className="flex items-center gap-5">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center shadow-sm ${log.status === 'success' ? 'bg-emerald-500/10 text-emerald-500' :
